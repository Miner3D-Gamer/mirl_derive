...
/// ```
#[proc_macro_attribute]
pub fn derive_possible_configured(args: TokenStream, input: TokenStream) -> TokenStream {
    let mut item = parse_macro_input!(input as DeriveInput);
    let FlagList(mut flags) = parse_macro_input!(args as FlagList);
...
/// ```
#[proc_macro_attribute]
pub fn derive_better_enum(args: TokenStream, input: TokenStream) -> TokenStream {
    let mut item = parse_macro_input!(input as DeriveInput);
    let FlagList(mut flags) = parse_macro_input!(args as FlagList);
    if let Some(err) = apply_derive(
        &mut item,
